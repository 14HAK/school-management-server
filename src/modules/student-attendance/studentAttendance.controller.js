...
import * as attendanceService from "./studentAttendance.service.js";
...
export const bulkCreateAttendance = asyncHandler(async (req, res) => {
  const result = await attendanceService.bulkCreateAttendance(req.body, req.user._id);
  res
    .status(201)
    .json(
      new ApiResponse(
        201,
        result,
        `Marked attendance for ${result.created.length} students${
